...
import { useSendRoomMessage, useSendPrivateMessage } from "@/hooks/useMessages";
...
export const MessageInput: React.FC = () => {
  ...
  const { sendMessage: sendRoomMessage } = useSendRoomMessage();
  const { sendMessage: sendPrivateMessage } = useSendPrivateMessage();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (message.trim()) {
      try {
        if (currentRoom) {
          // Send room message via socket (for real-time)
          socketService.sendRoomMessage(currentRoom.id, message.trim());
          // Also send via API for persistence and SWR cache update
          await sendRoomMessage(currentRoom.id, message.trim());
          socketService.stopTyping(currentRoom.id);
        } else if (currentPrivateChat) {
          // Send private message via API (SWR will handle cache update)
          await sendPrivateMessage(currentPrivateChat.id, message.trim());
